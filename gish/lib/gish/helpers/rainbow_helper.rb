module Gish
  module Helpers
    module RainbowHelper
      ...
      def paint(message: "", color: "white", bold: false)
        raise Gish::Exceptions::InvalidColorError.new color: color unless COLOR_CODES.has_key?(color.to_sym)
        raise Gish::Exceptions::InvalidColorError.new color: color if color == "white" && bold
      ...
      COLOR_CODES.keys.each do |color|
        define_method color do |message: "", bold: false|
          paint message: message, color: color, bold: bold
        end
      end
    end
  end
